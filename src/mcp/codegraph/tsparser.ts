...
type SyntaxNode = ParserNS.SyntaxNode;
...
export async function initTreeSitter(): Promise<boolean> {
  ...
  try {
    const mod: unknown = await import("web-tree-sitter");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ParserCtor = (mod as any).default ?? mod;
    const pkgRoot = path.dirname(require2.resolve("web-tree-sitter"));
    await ParserCtor.init({ locateFile: (f: string) => path.join(pkgRoot, f) });
    for (const g of new Set(Object.values(GRAMMAR_BY_EXT))) {
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        languages.set(g, await (ParserCtor as any).Language.load(wasmPath));
      } catch {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function parserFor(ext: string): any | null {
...
function descendants(n: SyntaxNode, pred: (c: SyntaxNode) => boolean): SyntaxNode[] {
  const out: SyntaxNode[] = [];
  const stack: SyntaxNode[] = [...n.namedChildren];
  while (stack.length) {
    const c = stack.pop()!;
...
function firstStringChild(n: SyntaxNode): string | null {
  for (const c of n.children) {
    if (c.type.includes("string")) return c.text;
  }
  return null;
}
...
// ---------- per-node extraction rules ----------

type Spec = [string, ImportRef["kind"]];

function nodeSpecifiers(n: SyntaxNode): Spec[] | null {
  switch (n.type) {
    // --- TS/JS/TSX ---
    ...
    case "call_expression": {
      ...
      if (fn !== "require" && fn !== "import") return null; // import() = dynamic import
    ...
    case "import_declaration": {
      ...
      if (strs.length) return strs.map((s) => [stripQuotes(s.text), "import"]);
 ...
 */
export function extractWithTreeSitter(filePath: string, content: string): ImportRef[] | null {
  ...
  const push = (specifier: string, kind: ImportRef["kind"]) => {
  ...
  const stack: SyntaxNode[] = [...root.namedChildren];
  while (stack.length) {
    const n = stack.pop()!;
    const specs = nodeSpecifiers(n);
    if (specs) for (const [s, k] of specs) push(s, k);
