...
import { writeManifest, hashContent, readManifest, detectDrift } from "../core/manifest.js";
const log = (msg, quiet) => { if (!quiet)
    console.log(msg); };
export function sync(options = {}) {
    const cwd = options.cwd ?? process.cwd();
    const { config, sources } = loadConfig(cwd);
    const targets = options.only ?? Object.keys(generators);
    ...
    if (!options.force) {
        if (drifted.length) {
            throw new Error(`Drift detected in:\n  ${drifted.join("\n  ")}\n\n` +
        ...
        if (unmanaged.length) {
            throw new Error(`Existing files would be overwritten:\n  ${unmanaged.join("\n  ")}\n\n` +
