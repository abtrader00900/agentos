...
export function createSupersearchServer(cwd = projectRoot()) {
    ...
    server.tool("supersearch_symbol", "Find function/class/method/interface definitions by name. Use before editing to locate symbols.", {
    ...
    server.tool("supersearch_history", "Find commits that changed a string (pickaxe search) — 'when was this introduced/removed?'.", {
    ...
    server.tool("supersearch_blame", "Per-line authorship of a file — who last touched each line and when.", {
        ...
        if (!lines.length)
            return { content: [{ type: "text", text: "No blame info." }] };
        return {
            content: [{
                    type: "text",
