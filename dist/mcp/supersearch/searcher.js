...
function rgAvailable() {
    if (rgCached === undefined)
        rgCached = spawnSync("rg", ["--no-require-git", "--version"], { stdio: "ignore" }).status === 0;
    return rgCached;
}
/** rg prints "./a/b.ts" (".\a\b.ts" on Windows); the builtin prints "a/b.ts" — make them identical */
const normalizePath = (f) => f.replace(/\\/g, "/").replace(/^\.\//, "");
function searchWithRg(opts) {
    ...
    if (r.status === 2)
        throw new Error(`ripgrep: ${(r.stderr ?? "").trim() || "search failed"}`);
    return (r.stdout ?? "")
        .split("\n")
        .filter(Boolean)
        .map((l) => {
...
export function searchTextBuiltin(opts) {
    ...
    const globMatches = (rel) => !globRx || globRx.test(opts.glob.includes("/") ? rel : path.basename(rel));
    const results = [];
    const max = opts.maxResults ?? 100;
    const walk = (dir, rel) => {
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory()) {
                if (SKIP_DIRS.has(e.name) || ignores.some((f) => f(r)))
                    continue;
                walk(path.join(dir, e.name), r);
            }
            else if (e.isFile()) {
                if (ignores.some((f) => f(r)))
                    continue;
                if (!globMatches(r))
                    continue;
                let buf;
                try {
                    if (statSync(path.join(dir, e.name)).size > MAX_FILE_BYTES)
                        continue;
                ...
                for (let i = 0; i < lines.length; i++) {
                    if (results.length >= max)
                        return;
                    if (rx.test(lines[i]))
                        results.push({ file: r, line: i + 1, text: lines[i].slice(0, 500) });
                }
            }
        }
    };
...
export function searchText(opts) {
