 ...
 */
const byField = (kind) => `kind: ${kind}\n  has: { field: name, pattern: $NAME }`;
// grammars without a `name` field: the identifier that is a direct child
const byChild = (kind, child) => `kind: ${kind}\n  has: { kind: ${child}, pattern: $NAME, stopBy: neighbor }`;
const RULES = [
    ...["TypeScript", "Tsx", "JavaScript"].flatMap((lang) => [
        { lang, kind: "function", rule: byField("function_declaration") },
        { lang, kind: "method", rule: byField("method_definition") },
        { lang, kind: "class", rule: byField("class_declaration") },
        ...(lang === "JavaScript" ? [] : [{ lang, kind: "interface", rule: byField("interface_declaration") }]),
    ]),
...
export function searchSymbols(opts) {
    const binary = astGrepBinary();
    if (!binary)
        throw new Error("ast-grep binary not found. Run: npm install @ast-grep/cli");
    ...
    for (const raw of items) {
        ...
        const relFile = rawFile
            ...
            : (opts.file ?? "");
        if (opts.file && relFile !== opts.file.replace(/\\/g, "/"))
            continue;
        results.push({
            file: relFile,
