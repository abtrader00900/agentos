...
export function createMemoryServer(dbPath = resolveDbPath()) {
    ...
    server.tool("memory_topics", "List all memory topics.", {}, async () => {
        const topics = store.topics();
        return { content: [{ type: "text", text: topics.length ? topics.join(", ") : "(empty)" }] };
    });
    server.tool("memory_export", "Export all memory as markdown.", {}, async () => ({ content: [{ type: "text", text: store.exportMarkdown() }] }));
    server.tool("memory_stats", "Memory stats: fact count, topic count.", {}, async () => {
        const s = store.stats();
